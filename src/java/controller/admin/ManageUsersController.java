    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (request.getQueryString() == null) {
                //ko filter
                users = dao.getAllUser(null, 0);
            } else {
                // co filter
                String userFullNameSearch = request.getQueryString().contains("name") ? request.getParameter("name").trim() : null;
                int roleId = 0;
                try {
                    roleId = Integer.parseInt(request.getParameter("role"));
                } catch (Exception e) {
                }
                users = dao.getAllUser(userFullNameSearch, roleId);
            }

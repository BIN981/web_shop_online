...
public class SliderDetailController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Slider slider = dao.getSliderById(sliderId, Constants.SLIDER_STATUS_ACTIVE);
        if(slider != null){
            List<Slider> sliderOthers = dao.getTop5Slider(sliderId);
            
            request.setAttribute("sliderOthers", sliderOthers);
            request.setAttribute("slider", slider);
            ProductCategoryUtil.setAttributeProductCategory(request, dao);
            CartUtil.setCounterCart(request, dao, -1);
            request.getRequestDispatcher("slider-detail.jsp").forward(request, response);
        } else {
            PrintWriter out = response.getWriter();
            out.print("Slider NOT FOUND!");
        }
    }
